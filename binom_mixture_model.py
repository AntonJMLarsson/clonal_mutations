...
class BinomialMixture():
    ...
    def modify_theta_list(self,N_ls, n_ls):
        self.theta_list[0] = 0.01
        Posterior = self.calc_Posterior(N_ls, n_ls)
        self.pi_list = pi_list = torch.sum(Posterior,1)/len(N_ls)
...
def estimate_clonal_marker_frequency(mut, sc_array, tc_array):
    ...
    BM_2 = BinomialMixture(n_components=2, verbose=False)
    try: 
        BM_2.fit(tc_tensor, sc_tensor)
    except ValueError:
        BM_2_pi = pd.Series([1.0,0.0])
        BM_2_pi.index = ['BM_2_pi_0', 'BM_2_pi_1']
        BM_2_theta = pd.Series([1e-6,1e-6])
        BM_2_theta.index = ['BM_2_theta_0', 'BM_2_theta_1']
        n_cells = pd.Series([len(tc_array_filtered)], index=['n_cells'])
        full_series = pd.concat([n_cells, BM_2_pi, BM_2_theta])
        full_series.name = mut
        return full_series
    ...
    full_series.name = mut
    return full_series
def find_posterior_probs(mut, sc_series, tc_series):
    sc_array = sc_series.values
    tc_array = tc_series.values
    cell_names = tc_series.index[tc_array>0]
    tc_array_filtered = tc_array[tc_array>0]
    sc_array_filtered = sc_array[tc_array>0]
    tc_tensor = torch.Tensor(tc_array_filtered)
    sc_tensor = torch.Tensor(sc_array_filtered)
    BM_2 = BinomialMixture(n_components=2, verbose=False)
    BM_2.fit(tc_tensor, sc_tensor)
    BM_2.modify_theta_list(tc_tensor, sc_tensor)
    pred = BM_2.predict(tc_tensor, sc_tensor)
    return pred, cell_names